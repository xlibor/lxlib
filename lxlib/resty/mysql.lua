...
local function read_result(self, est_nrows, rowAsList)
    ...
    while true do
        ...
        -- typ == 'DATA'
        if rowAsList then compact = true end
...
function _M.query(self, query, est_nrows, rowAsList)
    local bytes, err = send_query(self, query)
    if not bytes then
        return nil, "failed to send query: " .. err
    end

    return read_result(self, est_nrows, rowAsList)
end
