?>
<div data-bind="visible: loadingItem" class="loading"><?php 
echo(trans('administrator::administrator.loading'))
?></div>

<form class="edit_form" data-bind="visible: !loadingItem(), submit: saveItem">
	<h2 data-bind="text: $root[$root.primaryKey]() ? '<?php 
echo(trans('administrator::administrator.edit'))
?>' :
		'<?php 
echo(trans('administrator::administrator.createnew'))
?>'"></h2>
	...
	<!-- ko if: $root[$root.primaryKey]() -->
		<!-- ko if: $root.itemLink() -->
			<a class="item_link" target="_blank" data-bind="attr: {href: $root.itemLink()},
							text: '<?php 
echo(trans('administrator::administrator.viewitem', {single = config:getOption('single')}))
?>'"></a>
		<!-- /ko -->
	<!-- /ko -->
						...
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading, image: true,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php 
echo(trans('administrator::administrator.uploadimage'))
?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php 
echo(trans('administrator::administrator.imageuploading'))
?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				...
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php 
echo(trans('administrator::administrator.no_image_uploaded'))
?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
					...
					<div class="upload_container" data-bind="attr: {id: field_id}">
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php 
echo(trans('administrator::administrator.uploadfile'))
?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php 
echo(trans('administrator::administrator.fileuploading'))
?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				...
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php 
echo(trans('administrator::administrator.no_file_uploaded'))
?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
	...
	<div class="control_buttons">
		<!-- ko if: $root[$root.primaryKey]() -->
			<input type="button" class="btn btn-block btn-default" value="<?php 
echo(trans('administrator::administrator.close'))
?>"
				data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />

			<!-- ko if: actionPermissions.update -->
				<input type="button" class="btn btn-block btn-primary" value="<?php 
echo(trans('administrator::administrator.save'))
?>"
		...
		<!-- ko ifnot: $root[$root.primaryKey]() -->
			<input type="button" class="btn btn-block btn-default" value="<?php 
echo(trans('administrator::administrator.cancel'))
?>"
				data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
			<!-- ko if: actionPermissions.create -->
				<input type="submit" class="btn btn-block btn-primary" value="<?php 
echo(trans('administrator::administrator.create'))
?>"
